...
HAL_StatusTypeDef CANController::AddRxMessage(CANFrame *msg) {
    ...
    // Add message to map
    rx_messages_.emplace(msg->can_id, msg);
    num_msgs_++;
...
HAL_StatusTypeDef CANController::AddRxMessages(CANFrame *msg[], uint32_t num_msgs) {
    ...
    for (uint32_t i = 0; i < num_msgs; i++) {
        rx_messages_.emplace(msg[i]->can_id, msg[i]);
    }
    num_msgs_ += num_msgs;
...
HAL_StatusTypeDef CANController::AddFilterAll() {
    ...
    return HAL_CAN_ConfigFilter(devices_t[0]->hcan_, &sFilterConfig);
}
...
HAL_StatusTypeDef CANController::AddFilterId(uint32_t can_id, uint32_t id_type, uint32_t rtr_mode, uint32_t priority) {
    ...
    if ((filters_.size() >= NUM_FILTER_BANKS*2) && filters_.front().FilterMaskIdLow != 0)
        return HAL_ERROR;
...
HAL_StatusTypeDef CANController::AddFilterIdRange(uint32_t can_id, uint32_t range, uint32_t id_type, uint32_t rtr_mode, uint32_t priority) {
    if (filters_.size() >= NUM_FILTER_BANKS*2)
        return HAL_ERROR;
    if (range == 0)
        return HAL_ERROR;
...
//            HAL_CAN_ConfigFilter(device->hcan_, &filter);
    	...
    	{
    		HAL_CAN_ConfigFilter(devices_t[i]->hcan_, &filter);
    	}
    }
...
//            return HAL_ERROR;
    ...
    for (int i = 0; i < device_count; i++)
	{
		if (devices_t[i]->Send(msg) != HAL_OK)
			return HAL_ERROR;
	}
...
//        }
    for (int i = 0; i < device_count; i++)
	{
		if (devices_t[i]->hcan_ == hcan)
		{
			devices_t[i]->SetRxFlag();
			break;
		}
	}
}
